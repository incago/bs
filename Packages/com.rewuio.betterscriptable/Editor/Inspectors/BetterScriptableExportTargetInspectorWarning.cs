...
{
    [InitializeOnLoad]
    internal static class BetterScriptableExportTargetInspectorWarning
    {
        ...
        static BetterScriptableExportTargetInspectorWarning()
        {
        ...
        private static void DrawWarningIfNeeded(UnityEditor.Editor editor)
        {
            ...
            if (string.IsNullOrEmpty(assetPath)
                || BetterScriptableDocumentIO.IsDocumentPath(assetPath)
            ...
            EditorGUILayout.HelpBox(
                "This asset is a runtime export target for a BetterScriptable document. Direct Inspector edits can be overwritten the next time the source document is saved and exported.",
                MessageType.Warning);

            using (new EditorGUILayout.HorizontalScope())
            {
                if (GUILayout.Button("Open Source Document in BSE"))
                {
                    BetterScriptableWindow.OpenDocument(documentInfo.DocumentPath);
                }
        ...
        private static void EnsureCache()
        {
            ...
            {
                if (!BetterScriptableDocumentIO.TryRead(documentPath, out BetterScriptableDocument document, out _))
                {
                    continue;
                }
        ...
        private static IEnumerable<string> FindDocumentPaths()
        {
            ...
            foreach (string fullPath in Directory.GetFiles(
                         assetsDirectory,
                         "*" + BetterScriptableDocumentIO.Extension,
        ...
        private static string ResolveLinkedAssetGuid(BetterScriptableDocument document)
        {
            if (document == null)
            {
                return string.Empty;
