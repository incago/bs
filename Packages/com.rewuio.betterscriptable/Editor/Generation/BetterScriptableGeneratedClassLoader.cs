...
{
    internal static class BetterScriptableGeneratedClassLoader
    {
        ...
        private static readonly Regex FieldPattern = new Regex(
            @"new\s+BetterScriptableSchemaField\s*\{(?<body>.*?)\}",
        ...
        public static bool TryLoadFromSelection(out BetterScriptableGenerationRequest request, out string sourcePath, out string error)
        {
            request = null;
            ...
            out string error)
        {
            ...
            if (!TryParseFactory(factoryCode, out BetterScriptableDocumentSchema schema, out error))
            {
                return false;
            }
            ...
            request = new BetterScriptableGenerationRequest
            {
                AssetClassName = schema.AssetClassName,
                NamespaceName = string.IsNullOrEmpty(schema.NamespaceName)
                    ? BetterScriptableCodeGenerator.DefaultNamespace
        ...
        private static bool TryParseFactory(
            ...
            out string error)
        {
            schema = new BetterScriptableDocumentSchema();
            error = string.Empty;
        ...
        private static BetterScriptableSchemaField[] ParseAssetFields(string factoryCode)
        {
            int fieldsStart = factoryCode.IndexOf("Fields = new BetterScriptableSchemaField[]", StringComparison.Ordinal);
            int fieldsEnd = factoryCode.IndexOf("Tables =", fieldsStart, StringComparison.Ordinal);
            if (fieldsStart < 0 || fieldsEnd <= fieldsStart)
            {
                return Array.Empty<BetterScriptableSchemaField>();
            }
        ...
        private static BetterScriptableSchemaTable[] ParseTables(string factoryCode)
        {
            ...
            while (true)
            {
                int tableStart = tablesBlock.IndexOf("new BetterScriptableSchemaTable", searchIndex, StringComparison.Ordinal);
                if (tableStart < 0)
                {
                    break;
                }
                ...
                BetterScriptableSchemaTable table = ParseTable(tableBlock);
        ...
        private static BetterScriptableSchemaTable ParseTable(string tableBlock)
        {
            BetterScriptableSchemaTable table = new BetterScriptableSchemaTable();
        ...
        private static BetterScriptableSchemaField[] ParseFields(string block)
        {
